...
namespace {
std::string s_staged_body;
std::vector<uint8_t> s_persisted_grid;
}

unsigned long nowMs() { return s_fake_now_ms; }
void sleepMs(unsigned long ms) { advanceMs(ms); }
...
bool TemporaryBody::store(BodyReader& body) {
  ...
  for (;;) {
    ...
    if (n == 0) break;
    s_staged_body.append(chunk, n);
  }
  return !s_staged_body.empty();
}

bool TemporaryBody::replay(BodyFn on_body) {
  MemoryBodyReader body(s_staged_body.data(), s_staged_body.size());
  return on_body != nullptr && on_body(body);
}

void TemporaryBody::clear() { s_staged_body.clear(); }

bool TerrainCacheStore::load(void* data, size_t len) {
  if (data == nullptr || s_persisted_grid.size() != len) return false;
  memcpy(data, s_persisted_grid.data(), len);
  return true;
}
...
bool TerrainCacheStore::save(const void* data, size_t len) {
  ...
  s_persisted_grid.assign(bytes, bytes + len);
  return true;
}

void TerrainCacheStore::remove() { s_persisted_grid.clear(); }
...
void test_persisted_grid_round_trip_avoids_network(void) {
  const View v = makeView(kBaseLat, kBaseLon, kSpan);
  TEST_ASSERT_TRUE(ct::ensureGridPersisted(v.lat, v.lon, kRange, v.span));
  TEST_ASSERT_FALSE(pf::s_persisted_grid.empty());
  ct::clear();
  s_urls.clear();
  TEST_ASSERT_TRUE(ct::loadPersistedGrid(v.lat, v.lon, kRange, v.span));
  TEST_ASSERT_TRUE(ct::gridReady(v.lat, v.lon, kRange));
  TEST_ASSERT_EQUAL_UINT(0, s_urls.size());
}

void test_persisted_grid_rejects_other_view_keys(void) {
  const View v = makeView(kBaseLat, kBaseLon, kSpan);
  TEST_ASSERT_TRUE(ct::ensureGridPersisted(v.lat, v.lon, kRange, v.span));
  ct::clear();
  TEST_ASSERT_FALSE(ct::loadPersistedGrid(v.lat, v.lon, kRange + 1, v.span));
  TEST_ASSERT_FALSE(ct::loadPersistedGrid(v.lat + 0.01, v.lon, kRange, v.span));
  TEST_ASSERT_FALSE(ct::loadPersistedGrid(v.lat, v.lon, kRange, v.span + 1.0f));
}

void test_corrupt_version_and_truncated_cache_are_rejected(void) {
  const View v = makeView(kBaseLat, kBaseLon, kSpan);
  TEST_ASSERT_TRUE(ct::ensureGridPersisted(v.lat, v.lon, kRange, v.span));
  ct::clear();
  TEST_ASSERT_TRUE(pf::s_persisted_grid.size() > 8);
  const std::vector<uint8_t> valid = pf::s_persisted_grid;
  pf::s_persisted_grid[4] ^= 0x7f;  // packed format-version field
  TEST_ASSERT_FALSE(ct::loadPersistedGrid(v.lat, v.lon, kRange, v.span));
  pf::s_persisted_grid = valid;
  pf::s_persisted_grid[pf::s_persisted_grid.size() - 5] ^= 0x01;
  TEST_ASSERT_FALSE(ct::loadPersistedGrid(v.lat, v.lon, kRange, v.span));
  pf::s_persisted_grid = valid;
  pf::s_persisted_grid.resize(pf::s_persisted_grid.size() / 2);
  TEST_ASSERT_FALSE(ct::loadPersistedGrid(v.lat, v.lon, kRange, v.span));
}

void test_missing_cache_fetches_persists_and_cleans_staging(void) {
  const View v = makeView(kBaseLat, kBaseLon, kSpan);
  TEST_ASSERT_TRUE(ct::ensureGridPersisted(v.lat, v.lon, kRange, v.span));
  TEST_ASSERT_TRUE(s_urls.size() > 0);
  TEST_ASSERT_FALSE(pf::s_persisted_grid.empty());
  TEST_ASSERT_TRUE(pf::s_staged_body.empty());
}

void test_failed_fetch_is_nonfatal_and_leaves_no_staging_or_cache(void) {
  const View v = makeView(kBaseLat, kBaseLon, kSpan);
  s_outcomes = {0};
  TEST_ASSERT_FALSE(ct::ensureGridPersisted(v.lat, v.lon, kRange, v.span));
  TEST_ASSERT_TRUE(pf::s_persisted_grid.empty());
  TEST_ASSERT_TRUE(pf::s_staged_body.empty());
}
...
int main(int, char**) {
  ...
  RUN_TEST(test_persisted_grid_round_trip_avoids_network);
  RUN_TEST(test_persisted_grid_rejects_other_view_keys);
  RUN_TEST(test_corrupt_version_and_truncated_cache_are_rejected);
  RUN_TEST(test_missing_cache_fetches_persists_and_cleans_staging);
  RUN_TEST(test_failed_fetch_is_nonfatal_and_leaves_no_staging_or_cache);
