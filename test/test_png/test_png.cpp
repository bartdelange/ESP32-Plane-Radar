...
void test_production_owned_scratch_decodes_and_releases(void) {
  platform_png::setScratch(nullptr);
  TEST_ASSERT_TRUE(decodeFixture(fx::kDynamicHuffman));
  assertRasterShape(fx::kDynamicHuffman);
}

void test_no_scratch_is_a_clean_failure(void) {
  // An injected allocator can refuse the bounded workspace cleanly.
...
int main(int, char**) {
  ...
  RUN_TEST(test_production_owned_scratch_decodes_and_releases);
