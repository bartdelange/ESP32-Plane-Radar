 ...
 * point queries an elevation API would charge for. Device tiles are staged in
 * flash, then decoded after TLS closes; the raster is never held in RAM.
 *
...
/** Load the single persisted decoded grid when its complete view key matches. */
bool loadPersistedGrid(double center_lat, double center_lon,
                       uint8_t range_index, float half_span_km);

/** Atomically persist the current valid decoded grid. */
bool persistGrid();

/**
 * Synchronously resolve terrain before ADS-B starts: memory cache, persisted
 * cache, or one complete staged acquisition. Failure is non-fatal.
 */
bool ensureGridPersisted(double center_lat, double center_lon,
                         uint8_t range_index, float half_span_km);

/** Remove the persisted cache (normally unnecessary; mismatches are ignored). */
void invalidatePersistedGrid();
